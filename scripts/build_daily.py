...
# editable share-card source copied to the root alongside them on every build.
# og-image.png is the raster social share card the generated head actually
# references (X and other raster-only unfurlers exclude SVG cards). Like
# apple-touch-icon.png it is pinned at the root with no daily/ source, so the
# entry below is a presence check that fails the build loudly if the committed
# card ever goes missing.

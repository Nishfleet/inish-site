...
# app.js and styles.css are the legacy daily assets; og-image.svg and its fixed
# 1200x630 raster twin og-image.png are the social share card referenced by the
# generated head (the SVG stays reachable for compatibility, the PNG is what X
# and other raster-only unfurlers render), so both must be copied to the root
# alongside them on every build. apple-touch-icon.png is pinned at the root: it
# has no daily/ source, so the entry below is a presence check that fails the
# build loudly if the committed icon ever goes missing.

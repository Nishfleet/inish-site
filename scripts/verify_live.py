...
def main() -> int:
    ...
    # social share card and the iOS touch icon) are staged by deploy_daily.sh
    # and allowed through the worker allowlist, but nothing verified they
    # actually reached the live hostname; a deploy that dropped them again
    # would have passed verification. Byte-check them like the other payload
    # files. og-image.svg is only the card's design source and is deliberately
    # not served, so it sits in the removed list below.
    ...
    for path in ("/", "/app.js", "/styles.css", "/latest.json", "/feed.xml", "/og-image.png", "/apple-touch-icon.png", "/robots.txt", "/sitemap.xml", *fonts):
    ...
    removed = [
        "/llms.txt",
        "/og-image.svg",  # the SVG card is only a design source; the raster og-image.png is the served card
        "/archive",
        "/archive/",
        "/archive/index.html",
        "/daily/archive",

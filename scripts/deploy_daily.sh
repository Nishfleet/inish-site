...
# functions/policy.js (the single source of truth, which itself imports
# public-paths.json). The deploy payload ships both public-paths.json and
# functions/policy.js beside worker.js so both imports resolve at deploy
# time; tests enforce the copy line stays in exact sync with public-paths.json.
...
# Worker + wrangler config + public-paths.json + functions/policy.js are the
# live edge path (Workers assets + routes). worker.js imports the decision
# from functions/policy.js, which imports the data from public-paths.json;
# both data and logic modules ride beside worker.js in the deploy root.

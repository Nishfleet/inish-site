...
# card (og:image / twitter:image), its legacy SVG source kept reachable for
# compatibility, and the iOS home-screen icon. All three are pinned at the repo
# root and must ride in every temporary deploy payload.
...
class DeployDailyTests(unittest.TestCase):
    ...
    def test_payload_explicitly_carries_the_share_cards_and_home_screen_icon(self):
        ...
        for name in (RASTER_SHARE_CARD, SHARE_CARD, HOME_SCREEN_ICON):

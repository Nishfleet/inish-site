...
class BuildDailyTests(unittest.TestCase):
    ...
    def build(self):
        with patch.object(builder, "EDITIONS", self.editions), patch.object(builder, "DAILY", self.public):
            builder.main()
    ...
    def test_keeps_committed_root_assets_untouched(self):
        ...
        This is the regression test for the drift class that #27 and #33 fixed
        by hand-syncing the builder's daily/ mirror with merged root assets:
        every build silently reverted the merged fix until the next sync.
        """
        self.write(edition())

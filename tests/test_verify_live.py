...
class LiveVerifierTests(unittest.TestCase):
    ...
    def test_missing_share_asset_fails_verification(self):
        # The legacy share card is staged and middleware-allowed, but a deploy
    ...
    def test_missing_raster_share_card_fails_verification(self):
        # The head references the raster card for og:image/twitter:image, so a
        # deploy that drops og-image.png must fail loudly like any other
        # payload file — an SVG-only deploy can no longer pass as verified.

...
- The cross-repo "link inish.in back from product sites" item (backlog cd1a458ea0) is owned by the 0509 and tinystudio-io loops, not this repo: the footer attribution edits live in nish3451/0509 (`app/components/marketing-footer.tsx`, PR #599, commit `7b032018`) and nish3451/TinyStudio.io (`public/index.html` footer, PR #70, commit `f9f0b0f6`), both labeled "Nish's daily reads" → https://inish.in/. This repo has no code change for it; close the item when `curl -sL https://0509.io/` and `https://tinystudio.io/` both contain an href to `https://inish.in/`. Verified 2026-08-12: fresh curls of both homepages serve the `https://inish.in/` href, so the item's close condition is met and it is done live; the backlog item just needs to be ticked `[x]` so the controller stops re-opening inish-site lane 1 for it. siterep.net, seofixkit.com, and aiconverter.app stay out of scope until they gain attribution chrome.
